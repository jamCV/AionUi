...
const makeRepo = (conversation: TChatConversation, messages: TMessage[]): RepoMock => {
  ...
  return {
    ...
    getTurnSnapshotsByConversation: vi.fn(async () => []),
    updateTurnReviewStatus: vi.fn(async () => undefined),
    getTurnSnapshotFiles: vi.fn(async () => []),
  };
};
...
describe('TurnSnapshotCoordinator', () => {
  ...
  it('creates a turn snapshot from ACP diff messages', async () => {
    ...
    expect(repo.createdSnapshots[0]?.backend).toBe('acp:codex');
    expect(repo.createdSnapshots[0]?.reviewStatus).toBe('pending');
    expect(repo.createdSnapshots[0]?.files[0]?.filePath).toBe(filePath);
    expect(repo.createdSnapshots[0]?.files[0]?.beforeContent).toBe(beforeText);
    expect(repo.createdSnapshots[0]?.files[0]?.afterContent).toBe(afterText);
  });
  ...
  it('creates a turn snapshot from Codex unified diff messages', async () => {
    ...
    expect(repo.createdSnapshots[0]?.files[0]?.beforeContent).toBe(beforeText);
    expect(repo.createdSnapshots[0]?.files[0]?.afterContent).toBe(afterText);
    expect(repo.createdSnapshots[0]?.completionSource).toBe('task_complete');
  });
  ...
  it('does not persist a turn snapshot when no file changes are found', async () => {
    ...
    expect(repo.createdSnapshots).toHaveLength(0);
  });
  ...
  it('marks the snapshot as unsupported when a Codex diff cannot be reversed safely', async () => {
    ...
    expect(repo.createdSnapshots).toHaveLength(1);
    expect(repo.createdSnapshots[0]?.reviewStatus).toBe('unsupported');
    expect(repo.createdSnapshots[0]?.files[0]?.revertSupported).toBe(false);
    expect(repo.createdSnapshots[0]?.files[0]?.revertError).toBe('Failed to reverse codex turn diff patch.');
    expect(repo.createdSnapshots[0]?.files[0]?.beforeContent).toBeUndefined();
    expect(repo.createdSnapshots[0]?.files[0]?.afterContent).toBe('const version = 999;\n');
  });
});
