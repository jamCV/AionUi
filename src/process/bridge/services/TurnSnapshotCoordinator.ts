  ...
  private async extractAcpTurnFiles(message: IMessageAcpToolCall, workspace: string): Promise<ExtractedTurnFile[]> {
    const diffItemsByPath = new Map<
      string,
      {
        ...
        lastNormalizedDiff?: ToolCallNormalizedDiffItem;
    ...
    const normalizedDiffs = message.content.update.normalizedDiffs ?? [];
    ...
    for (const normalizedDiff of normalizedDiffs) {
      const currentEntry = diffItemsByPath.get(normalizedDiff.path) ?? { sourceMessageIds: new Set<string>() };
      diffItemsByPath.set(normalizedDiff.path, {
        ...currentEntry,
        lastNormalizedDiff: normalizedDiff,
        sourceMessageIds: currentEntry.sourceMessageIds.add(sourceMessageId),
      });
    }
    ...
    for (const [filePath, entry] of diffItemsByPath.entries()) {
      const beforeExists =
        entry.firstRawDiff?.oldText !== null && entry.firstRawDiff?.oldText !== undefined
          ? true
          : entry.firstRawDiff
            ? false
            : (entry.lastNormalizedDiff?.beforeExists ?? false);
      const afterExists =
        entry.lastRawDiff?.newText !== null && entry.lastRawDiff?.newText !== undefined
          ? true
          : entry.lastRawDiff
            ? false
            : (entry.lastNormalizedDiff?.afterExists ?? false);
      const beforeContent = beforeExists ? entry.firstRawDiff?.oldText : undefined;
      ...
      const fallbackUnifiedDiff =
        entry.lastNormalizedDiff?.unifiedDiff ??
        createUnifiedDiff(filePath, beforeExists, afterExists, beforeContent ?? '', afterContent ?? '');
      ...
      extractedFiles.push({
        ...
        beforeContent,
        afterContent,
        unifiedDiff: fallbackUnifiedDiff,
