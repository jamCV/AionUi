...
export const useConversations = () => {
  ...
  const pinnedConversations = groupedHistory.pinnedConversations;
  const workspaceGroups = groupedHistory.workspaceGroups ?? [];
  ...
  useEffect(() => {
    ...
    const currentDateGroups = new Set(
      workspaceGroups.flatMap((group) => group.dateGroups.map((dateGroup) => dateGroup.key))
    );
